...
useEffect(() => {
  ...
  const allOrders = useMemo(() => getOrdersByStatus("Reporté"), [getOrdersByStatus])

  // Filtrer pour n'avoir que les commandes confirmées
  const ordersConfirme = useMemo(() => {
   if (workerName) {
     return orders.filter(order =>
      order.status === "Reporté"&& order.confirmatrice === workerName
    );
  }
   else {
    return orders.filter((order) => order.status === "Reporté")
